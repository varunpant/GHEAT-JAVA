...
public class MercatorProjection implements Projections {
    ...
    public Size getTileMatrixSizePixel(int zoom) {
        ...
        return new Size(s.getWidth() * GHeat.SIZE, s.getHeight() * GHeat.SIZE);
    }
    ...
    public PointLatLng fromPixelToLatLng(DataPoint tlb, int zoom) {
        PointLatLng ret = new PointLatLng(0, 0, null, 0);
    ...
    public DataPoint fromPixelToTileXY(DataPoint pixelCoordinate) {
        return new DataPoint((long) (pixelCoordinate.getX() / GHeat.SIZE), (long) (pixelCoordinate.getY() / GHeat.SIZE));
    }

    @Override
    public DataPoint fromTileXYToPixel(DataPoint dataPoint) {
        return new DataPoint((dataPoint.getX() * GHeat.SIZE), (dataPoint.getY() * GHeat.SIZE));
    }
}
